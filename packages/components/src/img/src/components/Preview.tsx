...
export default defineComponent({
  ...
  setup(props, { expose }): () => VNode {
    ...
    const { rect, fit, getPreviewStartLocation } = useGetPreviewStartLocation(
    ...
    function closePreview() {
      ...
      if (props.toolbar.show || !_option.modal) {
        toolbarRef.value.style.transform = "translateX(-50%) translateY(50%)";
    ...
    onMounted(() => {
      ...
      imgReal.onload = () => {
        ...
        const scale =
          Math.min(clientWidth / imgRealWidth, clientHeight / imgRealHeight) /
          1.4;
        // 保存打开预览时的图片
        const imgItem = img.value;
        setImgStyleValues({
        ...
        imgItem.addEventListener(
          ...
          (e: WheelEvent) => {
            ...
            const toTopBorder = e.clientY - imgItem.getBoundingClientRect().top;
            scaleImg(e.deltaY > 0 ? "out" : "in", [toLeftBorder, toTopBorder]);
          },
          { passive: true },
        );
        ...
        if (props.modal) {
          maskRef.value &&
            maskRef.value.addEventListener("click", (e: MouseEvent) => {
              // 当用户点击遮罩左侧时, 切换到上一张图片
              if (
                e.clientX <
          ...
          // 恢复滚动的函数
          const resetBodyOverflow = (e: KeyboardEvent | MouseEvent | null) => {
            ...
            document.removeEventListener("keydown", resetBodyOverflow);
          };
          // 鼠标离开图片时恢复默认滚动
      ...
      if (props.toolbar.show) {
        _toolbarRef.value = createVNode(SePreviewToolbar, {
          ...
          pagination:
            _option.imgList.length > 1 ? props.toolbar.pagination : false,
          onZoom: scaleImg,
          onRotate: rotateImg,
          onFlip: flipImg,
          onReset: resetImg,
          ...
          class: !_option.modal ? "se-img-preview-toolbar-noModal" : "",
        });
    ...
    return () => {
      return (
        <div class="se-img-preview">
          {props.modal && <div class="se-img-preview-mask" ref={maskRef}></div>}
