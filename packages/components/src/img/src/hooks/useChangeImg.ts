...
export default function useOperate(
  ...
  function changeImg(
    ...
    if (nextIndex === "isFirst" || nextIndex === "isLast") {
      seMiniMeg({
        message:
          nextIndex === "isFirst" ? "已经是第一张了" : "已经是最后一张了",
        type: "warning",
        location: { x, y },
        duration: 1000,
      });
        ...
        changeOldImg = (() => {
          if (_option.animation === "fade") {
            ...
            return (isErr = false) => {
              ...
              locationInit();
              oldImgDom.style.left = type === "prev" ? "150vw" : "-150vw";
            };
          } else {
            return locationInit;
          }
    ...
    imgReal.onload = () => {
      ...
      if (_option.animation !== "none") {
        ...
        if (_option.animation === "fade") {
          ...
          imgStyle.setStyleValue("left", type === "prev" ? "-150vw" : "150vw");
  ...
  function userChangeImg(
    ...
    if (typeof specifyIndex !== "number" && _option.loop) {
      ...
      if (specifyIndex >= 0 && specifyIndex < total && specifyIndex !== index) {
